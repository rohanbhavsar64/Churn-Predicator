...
if o>40:
  ...
  # assuming hj is a dictionary with 'Runs' key
    ...
    fig = go.Figure(data=[go.Pie(values=hj['slice'], labels=hj['Runs'],texttemplate='%{value}', hovertemplate='<br>RUNS: %{label}<br>Slice: %{value}<br>%', marker_colors=colors)])
