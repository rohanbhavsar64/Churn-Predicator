...
if o==50:
    ...
    fig2 = px.pie(data_frame=lf_grouped, names='over_group', values='runs',custom_data=[lf_grouped['wickets']],hover_name='over_group', hover_data=['runs'],labels={'overs':'over_group'})
    fig2.update_traces(textinfo=f'%{labels}<br>R: %{value}<br>W: %{customdata[0]}')
...
wicket_text = temp_df['wickets_in_over'].astype(str)
wicket_y = temp_df['runs_after_over']+temp_df['wickets_in_over']*0.5  # adjust y-position based on wickets
wicket_y[wicket_y == temp_df['runs_after_over']] = None  # hide scatter points for 0 wickets
