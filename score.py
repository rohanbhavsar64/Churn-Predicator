...
if o==50:
    ...
    fig2 = px.pie(data_frame=lf_grouped, names='over_group', values='runs', text-template=f'%{label}<br>R: %{value}<br>W: %{customdata[0]}',custom_data=[lf_grouped['wickets']], hover_name='over_group')
    st.write(fig2)
gf=df
import pandas as pd
import numpy as np
