...
import plotly.graph_objects as go
import plotly.express as px
n=pipe.predict_proba(pd.DataFrame(columns=['batting_team','bowling_team','venue','score','wickets','runs_left','balls_left','crr','rrr','last_10','last_10_wicket'],data=np.array(gf.iloc[-1,:]).reshape(1,11))).astype(float)
